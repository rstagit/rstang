...
import com.v2ray.ang.service.RstaSpoofService
...
object CoreServiceManager {
    ...
    fun stopVService(context: Context) {
        ...
        val ctx = serviceControl?.get()?.getService()
        if (ctx != null) {
            val intent = Intent(ctx, RstaSpoofService::class.java).apply { action = "STOP" }
    ...
    private fun startContextService(context: Context) {
        ...
        if (MmkvManager.decodeSettingsBool(AppConfig.PREF_FRAGMENT_AUTO_SETTING, true)) {
            MmkvManager.encodeSettings(AppConfig.PREF_FRAGMENT_ENABLED, true)
            MmkvManager.encodeSettings(AppConfig.PREF_FRAGMENT_LENGTH, "1-1")
            MmkvManager.encodeSettings(AppConfig.PREF_FRAGMENT_INTERVAL, "0-0")
        ...
        if (RstaSpoofConfig.isSpoofTarget(config.server, config.serverPort)) {
