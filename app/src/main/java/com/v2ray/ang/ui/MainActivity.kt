...
class MainActivity : HelperBaseActivity(), NavigationView.OnNavigationItemSelectedListener {
    ...
    private fun setupViewModel() {
        ...
        mainViewModel.autoConnectFinished.observe(this) { found ->
            if (!found) {
                applyRunningState(false, mainViewModel.isRunning.value == true)
                toast(R.string.toast_auto_connect_no_valid_ping)
            }
        }
    ...
    private fun startV2Ray() {
        ...
        if (binding.switchAutoConnect.isChecked) {
            setTestState(getString(R.string.connection_test_testing))
            mainViewModel.startAutoConnectBestPing()
        } else {
